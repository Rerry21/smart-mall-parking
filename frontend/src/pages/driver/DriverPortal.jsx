...
function ActiveBookingBanner({ driverPhone }) {
  ...
  useEffect(() => {
    ...
    const fetchBooking = async () => {
      try {
        const res  = await fetch(`${API}/api/bookings/${driverPhone}`);
    ...
    fetchBooking();
    // Re-check every 30s in case a new booking is made
    const poll = setInterval(fetchBooking, 30000);
  ...
  const isUrgent    = secondsLeft !== null && secondsLeft <= 300 && secondsLeft > 0;  // under 5 min
  const isWarning   = secondsLeft !== null && secondsLeft <= 600 && secondsLeft > 300; // under 10 min

  const bgColor = isOverstay ? 'rgba(239,68,68,0.15)'
                : isUrgent   ? 'rgba(239,68,68,0.1)'
                : isWarning  ? 'rgba(245,158,11,0.1)'
                :              'rgba(59,130,246,0.1)';
  const borderColor = isOverstay ? 'rgba(239,68,68,0.5)'
                    : isUrgent   ? 'rgba(239,68,68,0.3)'
                    : isWarning  ? 'rgba(245,158,11,0.3)'
                    :              'rgba(59,130,246,0.3)';
  const textColor = isOverstay ? '#f87171'
                  : isUrgent   ? '#fca5a5'
                  : isWarning  ? '#fbbf24'
                  :              '#60a5fa';
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
        ...
        <div>
          <div style={{ fontFamily: "'Syne',sans-serif", fontSize: '15px', fontWeight: 700, color: '#fff' }}>
            {isOverstay ? 'Overstay! Fine Active' : `Your active booking — Slot ${booking.slotCode}`}
          </div>
          <div style={{ fontSize: '12px', color: 'rgba(255,255,255,0.45)', marginTop: '2px' }}>
            {isOverstay
              ? `KSh 300 fine per hour of overstay — please exit immediately`
              : `Paid KSh ${booking.amountPaid} · ${booking.duration} min session`
            }
          </div>
        </div>
      </div>
      <div style={{
        ...
        {isOverstay
          ? `+${formatCountdown(Math.abs(secondsLeft))} over`
...
const DriverPortal = () => {
  ...
  const handleBook = async () => {
    ...
    if (bookingPhone.length !== maxDigits) { setBookingError(`Enter a valid ${maxDigits}-digit phone number`); return; }
    if (!bookingDuration || bookingDuration < 10) { setBookingError('Minimum duration is 10 minutes'); return; }
    ...
    try {
      const res  = await fetch(`${API}/api/bookings`, {
        ...
        body: JSON.stringify({ driverPhone: fullPhone, slotCode: selectedSlot.number, duration: bookingDuration }),
      });
      const data = await res.json();
      if (res.ok) {
    ...
    } catch { setBookingError('Cannot connect to backend'); }
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0d0f14', fontFamily: "'DM Sans',sans-serif", color: '#f0f2f5' }}>
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 40px 60px' }}>
        ...
        {/* ── ACTIVE BOOKING BANNER ── */}
        {user?.phone && (
          <ActiveBookingBanner driverPhone={user.phone.replace(/^\+/, '')} />
        )}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(180px,1fr))', gap: '14px' }}>
          {filteredSlots.map((slot, i) => (
            <div key={slot.id} className="slot-card" onClick={() => slot.available && setSelectedSlot(slot)} style={{ padding: '22px 20px', borderRadius: '16px', border: slot.available ? '1px solid rgba(34,197,94,0.25)' : '1px solid rgba(239,68,68,0.2)', background: slot.available ? 'rgba(34,197,94,0.05)' : 'rgba(239,68,68,0.04)', cursor: slot.available ? 'pointer' : 'default', animation: `fadeUp 0.4s ${i * 0.04}s ease both`, boxShadow: slot.available ? '0 4px 20px rgba(34,197,94,0.06)' : 'none' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px' }}>
                ...
                <span style={{ padding: '3px 9px', borderRadius: '20px', fontSize: '10px', fontWeight: 700, background: slot.available ? 'rgba(34,197,94,0.15)' : 'rgba(239,68,68,0.12)', color: slot.available ? '#4ade80' : '#f87171' }}>{slot.available ? 'FREE' : 'TAKEN'}</span>
              </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: '24px' }}>
          <div style={{ background: '#13161b', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '24px', padding: '36px', width: '100%', maxWidth: '420px', boxShadow: '0 32px 80px rgba(0,0,0,0.6)' }}>
            ...
            <div style={{ marginBottom: '18px' }}>
              ...
              <input type="number" min="10" max="480" value={bookingDuration} onChange={e => setBookingDuration(Number(e.target.value))} style={{ ...modalInput, marginTop: '4px' }} placeholder="Or type custom minutes..." />
            ...
            <div style={{ marginBottom: '24px' }}>
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                ...
                <input type="tel" value={bookingPhone} onChange={e => { const val = e.target.value.replace(/\D/g, ''); const max = PHONE_LIMITS[bookingCC] || 10; if (val.length <= max) setBookingPhone(val); }} placeholder={`${PHONE_LIMITS[bookingCC] || 10} digits`} style={{ ...modalInput, flex: 1 }} />
              </div>
            ...
            <button onClick={() => { setSelectedSlot(null); setBookingError(''); setBookingMsg(''); setBookingPhone(''); }} style={{ width: '100%', padding: '13px', borderRadius: '14px', border: 'none', background: 'transparent', color: 'rgba(255,255,255,0.35)', fontFamily: "'DM Sans',sans-serif", fontSize: '14px', cursor: 'pointer' }}>Cancel</button>
          </div>
        </div>
      )}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: '24px' }}>
          <div style={{ background: '#13161b', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '24px', padding: '36px', width: '100%', maxWidth: '420px', boxShadow: '0 32px 80px rgba(0,0,0,0.6)', maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ textAlign: 'center', marginBottom: '24px' }}>
              ...
              {profilePhoto && <button onClick={handleRemovePhoto} style={{ display: 'block', margin: '8px auto 0', fontSize: '11px', color: '#f87171', background: 'transparent', border: 'none', cursor: 'pointer' }}>Remove photo</button>}
              ...
              <div>
                <div style={{ marginBottom: '16px' }}>
                  ...
                  <div style={{ display: 'flex', gap: '8px' }}>
                    ...
                    <input type="tel" value={editPhone} placeholder="New phone number" onChange={e => { const val = e.target.value.replace(/\D/g, ''); const max = PHONE_LIMITS[editPhoneCC] || 10; if (val.length <= max) setEditPhone(val); }} style={{ ...modalInput, flex: 1 }} />
                  </div>
                </div>
